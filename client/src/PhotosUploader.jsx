...
export default function PhotosUploader({ addedPhotos, onChange }) {
    ...
    async function addPhotoByLink(ev) {
        ev.preventDefault();
        try {
            // Send the link to the server to download the image
            const { data: filename } = await axios.post('http://localhost:4000/upload-by-link', { link: photoLink });
            console.log('Uploaded by link:', filename); // Debugging line
            onChange(prev => [...prev, filename]);
            setPhotoLink('');  // Reset input field
        } catch (error) {
            console.error('Error uploading photo by link:', error.response?.data || error.message);
        }
    }
    ...
    function uploadPhoto(ev) {
        ...
        .then(response => {
            const { data: filenames } = response; // Server should return array of filenames
            console.log('Uploaded files:', filenames); // Debugging line
    ...
    // Remove photo by filename
    function removePhoto(ev, filename) {
        ev.preventDefault();
        onChange(addedPhotos.filter(photo => photo !== filename));
    }

    // Set a photo as the main one
...
PhotosUploader.propTypes = {
    addedPhotos: PropTypes.arrayOf(PropTypes.string).isRequired,  // Array of strings (filenames or links)
